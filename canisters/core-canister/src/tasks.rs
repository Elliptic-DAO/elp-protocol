...
pub enum TaskType {
    ProcessLogic,
    ProtocolBalanceUpdate,
    FetchPrice,
    CheckLeveragePositions,
    CloseLeveragePosition(LeveragePosition),
}
...
impl TaskQueue {
    ...
    pub fn get_task_queue(&self) -> Vec<Task> {
...
pub fn get_task_vec() -> Vec<Task> {
    TASKS.with(|t| t.borrow().get_task_queue())
}
